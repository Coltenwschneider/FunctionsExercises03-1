...
/*function reverseCharacters(stringReversed){
  return stringReversed.split("").reverse().join("");
}
...
function reverseCharacters(stringReversed){
  if (typeof stringReversed === 'string'){
    return stringReversed.split("").reverse().join("");
  }
  else if(typeof stringReversed === 'number'){
    return Number(stringReversed.toString().split("").reverse().join(""));
  }
}
...
function newFunction(arrayTest){
  let newArray = []
...
let myReturnedArray=newFunction(arrayTest1)
console.log(newFunction(arrayTest3))
